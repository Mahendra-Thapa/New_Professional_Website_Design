        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-7">
            {SERVICE_PAGES.map(({ slug, label, hero, subServices, gradient }, i) => {
              ...
              const Icon = (Icons as unknown as Record<string, React.ComponentType<{ size?: number; className?: string }>>)[iconName] ?? Icons.Code2;
              return (
                <motion.div key={slug} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }}
                  ...
                  <div className="p-6">
                    ...
                    <Link href={`/services/${slug}`} className="inline-flex items-center gap-1.5 text-[#0D47A1] font-semibold text-sm group-hover:gap-3 transition-all">
